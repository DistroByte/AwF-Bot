...
module.exports = class {
  ...
  async run(oldMember, newMember) {
    const fRoleIDs = Object.keys(this.client.config.factorioRoles).map((name) => this.client.config.factorioRoles[name].id)
    if (newMember.roles.cache.some((r) => fRoleIDs.includes(r.id))) {
      ...
      const authRoleIDs = fRoleIDs.filter((role) => memberRoleIDs.includes(role))
      let authRoles = []
