...
module.exports = class {
  ...
  async run(message) {
    ...
    if (message.content.match(new RegExp(`^<@!?${client.user.id}>( |)$`))) {
      if (message.guild) {
        return message.channel.send(`Hello **${message.author.username}**, my prefix on this server is \`${data.guild.prefix}\`.Use \`${data.guild.prefix}help\` to get a list of the commands!`);
      } else {
