	...
	async _getData() {
		...
		let promiseArray = this._servers.map(async (server) => {
			if (server.playercount !== 0) {
				...
				try {
					rcon.rconCommand(`/sc global.ext = game.json_to_table('${JSON.stringify({
						var: {
							server_ups: server.ups,
							status: "Online"
						},
						servers: [
							{ name: "All-Weekend Factorio", welcome: "Welcome to All-Weekend Factorio servers!", reset_time: "Depends on server", branch: "Unknown" }
						],
						current: 1
					})}')`, server.discordid).then(() => { }).catch(() => { })
				} catch { }
			}
			return server
		})
