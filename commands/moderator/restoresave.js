...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (
      ...
      if (!args[1]) {
        // no second argument, only server name
        let choiceEmbed = new Discord.MessageEmbed()
          .setTitle("Server Rollback Choices")
          .setDescription(
            "Choices of a Factorio Server Rollback. This shows **all** .zip files, some may not be Factorio saves. **Most are sorted by date last modified, check the date in the field below the save name**"
          )
          .setColor("GREEN")
          .setAuthor(
            `${message.guild.me.displayName} Help`,
        ...
        dirData = await sortModifiedDate(dir).catch((err) => {
          console.log(err);
        });

        for (let i = 0; i < 25; i++) {
          // max number of fields in a Discord Embed is 25
          if (!dirData[i]) break;
          if (dirData[i] && dirData[i].endsWith(".zip")) {
            let data = fs.statSync(dir + "/" + dirData[i]);
            let date = new Date(data.birthtimeMs);
            choiceEmbed.addField(
              `\`${dirData[i].split(".")[0]}\``,
              `Save created on: ${date.toUTCString()}`
            );
          }
        }
      ...
      if (args[0] && args[1]) {
        // both server name and save name are provided
        let choiceEmbed = new Discord.MessageEmbed()
          ...
          .addField(
            `Rolled back server \`${args[0]}\` to save \`${args[1]}\``,
            "You may need to start the server with a separate command"
          );
        ...
        await runShellCommand("test -w .; echo $?")
          ...
          .catch((e) => {
            return message.channel.send(
              `error testing file permissions: \`${e}\``
            );
          });
        ...
        runShellCommand(command.join(" && ")).catch((e) => {
