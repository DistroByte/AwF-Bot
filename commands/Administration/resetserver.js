...
class Linkme extends Command {
  constructor(client) {
    super(client, {
      ...
      usage: "<#channel> [--scenario <Scenario name>][File attatchment for map gen settings]",
  ...
  async run(message, args) {
    if (!message.mentions.channels.first()) return message.reply("No channel to reset provided!")
    args.shift(); // remove mention
    let server = serverJS.find((server) => server.discordid === message.mentions.channels.first().id)
    ...
    // back up saves
    const saves = fs.readdirSync(`${this.client.config.serverpath}/${server.path}/saves`).map((save) => `${this.client.config.serverpath}/${server.path}/saves/${save}`)
    const latestSavePath = saves[0]
    // if there are saves, back up latest and remove all after
    if (latestSavePath) {
      const latestSave = latestSavePath.slice(latestSavePath.lastIndexOf("/") + 1, latestSavePath.indexOf('.'))
      if (!fs.existsSync(`${this.client.config.archivePath}/${server.path}/`))
        fs.mkdirSync(`${this.client.config.archivePath}/${server.path}/`)
      fs.copyFileSync(latestSavePath, `${this.client.config.archivePath}/${server.path}/${latestSave}_${moment().format("YYYY-MM-DD-mm-ss")}.zip`)
      saves.forEach((savePath) => fs.rmSync(savePath))
    }

    // remove stats
    ServerStatistics.findOneAndReplace({ serverID: server.discordid }, {
    ...
    factorio.on('close', (code) => {
      ...
      childprocess.spawn(`./factorio-init/factorio`, ['start'], { detatched: true, cwd: `${this.client.config.serverpath}/${server.path}` } )
      setTimeout(() => {
        runShellCommand(`${this.client.config.serverpath}/${server.path}/factorio-init/factorio status`)
          .then((out) => {
