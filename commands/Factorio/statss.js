...
class Linkme extends Command {
	...
	async run(message, args) {
		...
		if (!stats) return message.channel.send(`\`${args[0]}\` is an invalid server!`)
		const statsembed = new MessageEmbed()
			.setAuthor(message.guild.name, message.guild.iconURL())
			.setColor(this.client.config.embed.color)
			.setFooter(this.client.config.embed.footer);
		statsembed.addField("Rockets launched", stats.rocketLaunches)
		const research = stats.completedResearch?.reduce((research, max) => research.level > max.level ? research : max, {name: "", level: 0})
		if (research.level)
			statsembed.addField("Highest research", `Name: ${research.name}\nLevel: ${research.level}`)
		if (stats.completedResearch?.length)
			statsembed.addField("Total amount of research completed", stats.completedResearch.length)
		let evolution = await rcon.rconCommand("/evolution", serverid).then(r=>r.resp)
