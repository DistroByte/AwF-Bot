...
class Linkme extends Command {
  ...
  async run(message, args) {
    if (!args[0]) return message.channel.send("No linking code supplied! Get one from in-game")
    const linkID = this.client.cache.linkingCache.get(`${args[0]}`)
    if (!linkID) return message.channel.send(`Key ${args[0]} is invalid or expired. Keys should be in the format \`123456\``)
    ...
    embed.setDescription("You have chosen to link youself. React with ✅ to confirm, ❌ to cancel. Will time out in 90s")
    ...
    try {
      let user = await this.client.findOrCreateUser({id: message.author.id, guildID: message.guild.id})
