    ...
    run: async (client, message, args) => {
        ...
        if (!args[1]) { // if the server name is provided but no 2nd argument, searches for generic server data
            if (message.mentions.users.first()) {
                ...
                else return message.channel.send(`\`${message.mentions.members.first().user.username}\` is already linked under username ${res.factorioName}!`); 
            } else {
                ...
                return message.channel.send(`\`${args[0]}\` is already linked under Discord tag \`${discUser.username}\`!`);
            }
        }
    }
}