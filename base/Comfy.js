...
class Comfy extends Client {
  constructor(options) {
    ...
    setInterval(() => {
      this.serverQueues.forEach((server) => {
        ...
        if (message.length) {
          this.channels.cache
            .get(server.server.discordid)
            ?.send(message)
            .then(() => (server.sendingMessage = false));
        } else {
  ...
  async findUserFactorioName(factorioname, isLean) {
    let userData = isLean
      ? await this.usersData.findOne({ factorioName: factorioname }).lean()
      : await this.usersData.findOne({ factorioName: factorioname });
    if (!isLean && userData?.id)
    ...
    if (this.config.moderatorchannel) {
      const guild = this.guilds.cache.forEach((guild) =>
        guild.channels.cache.forEach(
          (channel) =>
            channel.id === this.config.moderatorchannel && channel.send(message)
        )
      );
    }
  }
