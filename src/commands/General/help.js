...
class Help extends Command {
  ...
  async run(message, args, data) {
    ...
    if (args[0]) {
      const isCustom = data.guild.customCommands
        ? data.guild.customCommands.find((c) => c.name === args[0])
        : false;
    ...
    if (message.guild) {
      if (data.guild.customCommands.length > 0) {
        embed.addField(
          message.guild.name +
            " | Custom commands - (" +
            data.guild.customCommands.length +
            ")",
          data.guild.customCommands.map((cmd) => `\`${cmd.name}\``).join(", ")
        );
      }
    }
    ...
    embed.addField(
      "\u200B",
      `[Dashboard](https://dashboard.dbyte.xyz) ● [Donate](https://www.patreon.com/distrobyte) ● [Invite](${this.client.config.inviteURL}) ● [Support](${this.client.config.supportURL}) ● [Github](https://www.github,com/DistroByte)`
    );
    embed.setAuthor(
