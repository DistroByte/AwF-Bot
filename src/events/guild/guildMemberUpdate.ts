...
export default async (client: Comfy, _oldMember: GuildMember, newMember: GuildMember) => {
	const user = await client.findOrCreateUser({ id: newMember.id })
	const factorioRoleIDs = Object.keys(client.config.factorioRoles).map(name => client.config.factorioRoles[name].id)
	if (user.factorioName && newMember.roles.cache.some((r) => factorioRoleIDs.includes(r.id))) {
		const memberRoleIDs = newMember.roles.cache.map((role) => role.id);
		const shouldHaveRoleIDs = factorioRoleIDs.filter(r => memberRoleIDs.includes(r))
		...
		if (rolesToUnassign.length) {
			rcon.rconCommandAll(`/interface Roles.unassign_player('${user.factorioName}', {'${rolesToUnassign.join("', '")}'}, "JammyBot")`)
		}
		factorioRoles.forEach((role) => {
			rcon.rconCommandAll(`/interface Roles.assign_player('${user.factorioName}', '${role}', "JammyBot")`)
		})
